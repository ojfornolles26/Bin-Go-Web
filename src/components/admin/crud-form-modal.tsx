...
interface CrudFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (record: CrudRecord) => void;
  editingRecord?: CrudRecord | null;
}
...
export default function CrudFormModal({
  ...
  const [formData, setFormData] = useState<CrudRecord>({
    id: '',
    location: '',
    barangay: '',
    reporter: '',
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div>
            <h3 className="text-base font-bold text-slate-900">
              {editingRecord ? 'Edit Record (Update)' : 'Create New Record (Create)'}
            </h3>
            <p className="text-xs text-slate-500">
              {editingRecord ? `Modifying ticket ${editingRecord.id}` : 'Fill in record fields to insert into Supabase'}
            </p>
          </div>
          <button
            onClick={onClose}
